...
class MessagesCountCog(commands.Cog):
    ...
    async def on_message(self, message: discord.Message):
        ...
        if highest_role_that_can_be_awarded and (current_role is None or current_role.id != highest_role_that_can_be_awarded):
            ...
            if new_role:
                ...
                for role in message.author.roles:
                    if role.id in self.role_rewards.values():
                        await message.author.remove_roles(role, reason="New role awarded.")
                
                await message.author.add_roles(new_role)
                await message.channel.send(f"Congratulations! You have reached {user_logger['count']} messages and have been awarded the {new_role.name} role.", mention_author=False)

    @commands.command()
    async def count(self, ctx: commands.Context, member: Union[discord.Member, None]):
        """
...
async def setup(bot: commands.Bot):
    await bot.add_cog(MessagesCountCog(bot))
